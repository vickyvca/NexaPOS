...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($_POST['item_id'] as $idx => $itemId) {
            ...
            $subtotal = $qty * $price;
            $stmt = $pdo->prepare("INSERT INTO purchase_items(purchase_id,item_id,qty,price,subtotal) VALUES(?,?,?,?,?)");
            $stmt->execute([$pid, $itemId, $qty, $price, $subtotal]);
            if ($_POST['status'] === 'posted') {
                ...
                $pdo->prepare("INSERT INTO stock_moves(item_id, ref_type, ref_id, date, qty_in, qty_out, note, created_by) VALUES(?,?,?,?,?,?,?,?)")
                    ->execute([$itemId, 'purchase', $pid, $_POST['date'], $qty, 0, 'Pembelian', current_user()['id']]);
            }
        }
        if ($_POST['status'] === 'posted') {
