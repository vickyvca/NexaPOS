...
$items = $pdo->query("SELECT id,code,barcode,name,sell_price,sell_price_lv2,sell_price_lv3,stock FROM items WHERE is_active=1 ORDER BY name LIMIT 50")->fetchAll();
$cart = $_SESSION['cart'];
$totalInit = 0;
...
function posApp() {
    return {
        ...
        scanCode: '',
        cart: [],
        itemsData: <?= json_encode(array_map(function($i){ return ['id'=>$i['id'],'code'=>$i['code'],'name'=>$i['name'],'l1'=>$i['sell_price'],'l2'=>$i['sell_price_lv2'],'l3'=>$i['sell_price_lv3']]; }, $items)); ?>,
        discountTotal: 0,
        tax: 0,
        cashPaid: 0,
        paymentMethod: 'cash',
        ...
        init() {
            ...
            this.cart = this.cart.map(c => ({...c, id: c.id ?? c.item_id ?? null, l1: c.price ?? c.l1 ?? 0, l2: c.l2 ?? c.sell_price_lv2 ?? 0, l3: c.l3 ?? c.sell_price_lv3 ?? 0, discount: c.discount ?? 0, qty: c.qty ?? 1, level: c.level ?? 1}));
            this.recalc();
            this.syncCardPrices();
        },
        addByCode() {
        ...
        searchSuggest(){
            ...
            this.suggestions = this.itemsData.filter(i => i.code.toLowerCase().includes(term) || i.name.toLowerCase().includes(term)).slice(0,8);
        },
        addItem(item) {
            ...
            if (existing) {
                ...
                this.cart.push({id:item.id, name:item.name, price, qty:1, discount:0, level:this.priceLevel, l1:item.l1, l2:item.l2, l3:item.l3});
            }
            this.recalc();
        },
        ...
        incQty(idx,delta){ const it=this.cart[idx]; if(!it) return; it.qty += delta; if(it.qty<=0){ this.cart.splice(idx,1);} this.recalc(); },
        ...
        handleMoneyInput(field, ev){
            const raw = (ev.target.value||'').toString().replace(/[^0-9]/g,'');
            const num = raw ? parseInt(raw,10) : 0;
            this[field] = num;
            ev.target.value = num ? num.toLocaleString('id-ID') : '';
            this.calcPayment();
        },
        syncCardPrices(){
