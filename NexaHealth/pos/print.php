...
$items = $pdo->prepare("SELECT si.*, i.name, b.batch_no FROM sale_items si JOIN items i ON i.id=si.item_id LEFT JOIN batches b ON b.id=si.batch_id WHERE sale_id=?");
$items->execute([$id]);
...
<div class="receipt">
    ...
    <table>
        ...
        <?php if (!empty($d['batch_id']) || !empty($d['expiry'])): ?>
        <tr>
            <td colspan="2" class="text-right">Batch: <?= htmlspecialchars($d['batch_id'] ? $d['batch_id'] : ($d['batch_no'] ?? '-')); ?><?php if (!empty($d['expiry'])): ?> | Exp: <?= htmlspecialchars($d['expiry']); ?><?php endif; ?></td>
        </tr>
        <?php endif; ?>
        <?php endforeach; ?>
    </table>
