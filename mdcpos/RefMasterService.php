...
class RefMasterService {
    ...
    // --- READ ONE (Ambil Satu Data berdasarkan KODEJN) ---
    public function readOneJenis(string $kodeJN) {
        try {
            $sql = "SELECT KODEJN, KETERANGAN FROM REF_JENIS WHERE KODEJN = ?";
            $stmt = $this->conn->prepare($sql);
            ...
            if (!$row) {
                return ['status' => 'info', 'message' => 'Data tidak ditemukan.'];
            }
