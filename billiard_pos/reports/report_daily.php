...
check_login(); // kasir diizinkan lihat harian
...
$maintStmt = $pdo->prepare("SELECT ml.*, t.name AS table_name, u.username FROM maintenance_logs ml JOIN billiard_tables t ON ml.table_id = t.id LEFT JOIN users u ON ml.operator_id = u.id WHERE DATE(ml.start_time)=? ORDER BY ml.start_time DESC");
$maintStmt->execute([$date]);
$maintLogs = $maintStmt->fetchAll();

$expenseStmt = $pdo->prepare("SELECT e.*, a.name AS account_name, u.username FROM expenses e JOIN accounts a ON e.account_id = a.id LEFT JOIN users u ON e.operator_id = u.id WHERE DATE(e.expense_time)=? ORDER BY e.expense_time DESC");
...
$journalBreak = [];
$jstmt = $pdo->prepare("SELECT a.type, SUM(CASE WHEN j.type='in' THEN j.amount ELSE 0 END) AS masuk, SUM(CASE WHEN j.type='out' THEN j.amount ELSE 0 END) AS keluar FROM journals j JOIN accounts a ON j.account_id = a.id WHERE DATE(j.txn_time) = ? GROUP BY a.type");
$jstmt->execute([$date]);
foreach ($jstmt as $j) {
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2">
        ...
        <a class="btn btn-success btn-sm" target="_blank" href="/billiard_pos/reports/print_daily.php?date=<?php echo htmlspecialchars($date); ?>">Export PDF / Print</a>
    </div>
</div>
...
<div class="row mb-3">
    <div class="col-md-4">
        <div class="card bg-secondary text-light"><div class="card-body"><div>Pengeluaran Hari Ini</div><div class="fs-4"><?php echo format_rupiah($total_expense); ?></div></div></div>
    </div>
</div>
<div class="row mb-3">
...
<div class="card bg-secondary text-light">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            <a class="btn btn-sm btn-outline-light" href="?date=<?php echo htmlspecialchars($date); ?>&export=csv">Export CSV</a>
        </div>
    </div>
    <div class="card-body">
        <table class="table table-dark table-striped data-table">
            <thead><tr><th>#</th><th>Jam (Start-End)</th><th>Customer</th><th>Meja</th><th>Durasi</th><th>Paket</th><th>Billing</th><th>POS</th><th>Extra</th><th>Diskon</th><th>Grand</th><th>Bayar</th><th>POS Detail</th><th>Nota</th></tr></thead>
            <tbody>
            <?php $no=1; foreach ($orders as $o): ?>
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-sm btn-outline-light" target="_blank" href="/billiard_pos/pos/print_receipt.php?order_id=<?php echo $o['id']; ?>">Cetak Ulang</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
